...
public class ChangeUserNameDialog extends Dialog implements View.OnClickListener {
    ...
    public ChangeUserNameDialog(Context context, String oldText, TextView textView) {
        super(context);
    ...
    public void onClick(View v) {
        switch (v.getId()) {
            case R.id.d_user_change_name_button_save:
                ...
                if(text.length() > 40) {
                    Toast.makeText(getContext(), "Nazwa nie może być dłuższa niż 30 znaków", Toast.LENGTH_SHORT).show();
                    return;
                }
                textView.setText(text);
                RequestExecutor.offerRequest(new ChangeUserName(text));
